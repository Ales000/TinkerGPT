...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask, layer_idx=0, epoch=0):
        if epoch == 0 and layer_idx == 0: print("    [DEBUG] Вход в EncoderLayer 0...")
        x = x + self.attn(self.norm1(x), self.norm1(x), self.norm1(x), mask)
        x = x + self.ff(self.norm2(x))
        if epoch == 0 and layer_idx == 0: print("    [DEBUG] Выход из EncoderLayer 0.")
        return x
...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, num_heads, d_ff):
        ...
        self.ff = PositionWiseFeedForward(d_model, d_ff)
    def forward(self, x, enc_output, src_mask, tgt_mask, layer_idx=0, epoch=0):
        if epoch == 0 and layer_idx == 0: print("    [DEBUG] Вход в DecoderLayer 0...")
        ...
        if epoch == 0 and layer_idx == 0: print("    [DEBUG] Выход из DecoderLayer 0.")
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, epoch=0):
        if epoch == 0: print("  [DEBUG] Начало Transformer.forward()...")
        src_mask = self.make_src_mask(src)
        tgt_mask = self.make_tgt_mask(tgt)
        src = self.pos_encoder(self.embedding(src))
        tgt = self.pos_encoder(self.embedding(tgt))
        if epoch == 0: print("  [DEBUG] Проходим через Encoder...")
        for i, layer in enumerate(self.encoder_layers):
            src = layer(src, src_mask, layer_idx=i, epoch=epoch)
        if epoch == 0: print("  [DEBUG] Проходим через Decoder...")
        for i, layer in enumerate(self.decoder_layers):
            tgt = layer(tgt, src, src_mask, tgt_mask, layer_idx=i, epoch=epoch)
        if epoch == 0: print("  [DEBUG] Вычисляем финальные логиты...")
        output = self.fc_out(tgt)
        if epoch == 0: print("  [DEBUG] Transformer.forward() завершен.")
        return output
...
if os.path.exists(MODEL_PATH) and os.path.exists(TOKENIZER_PATH):
    ...
    print(f"[DEBUG] Используемое устройство: {device}")
    ...
    for epoch in range(epochs):
        print(f"\n[DEBUG] Начало Эпохи {epoch+1}...")
        
        optimizer.zero_grad()
        print("[DEBUG] Шаг 1: Очистка градиентов (zero_grad).")
        
        output = model(src_data, tgt_data, epoch=epoch)
        print("[DEBUG] Шаг 2: Прямой проход (model forward).")
        
        loss = criterion(output.view(-1, vocab_size), y_labels.view(-1))
        print(f"[DEBUG] Шаг 3: Расчет потерь (criterion). Loss: {loss.item():.4f}")
        
        loss.backward()
        print("[DEBUG] Шаг 4: Обратный проход (backward).")
        
        optimizer.step()
        print("[DEBUG] Шаг 5: Обновление весов (optimizer step).")
        
        if (epoch + 1) % 100 == 0:
            print(f"--- Эпоха {epoch+1}/{epochs}, Потери: {loss.item():.4f} ---")

        if epoch >= 0: # Условие для остановки после первой эпохи
            print("\n[DEBUG] Первая эпоха завершена. Прерываем для анализа.")
            break
...
# --- Логика общения закомментирована для отладки ---
# SOS_ID = tokenizer.token_to_id["<sos>"]
# EOS_ID = tokenizer.token_to_id["<eos>"]
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# model.to(device)
